...
resource "aws_iam_policy_attachment" "iam_system_policy_attachment" {
  ...
  roles      = [aws_iam_role.iam_role.name]
  policy_arn = var.use_service_role_arn ? 
               "arn:aws:iam::aws:policy/service-role/${var.custom_policy_names[count.index]}" : 
               "arn:aws:iam::aws:policy/${var.custom_policy_names[count.index]}"
}
